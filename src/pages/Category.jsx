...
export default function Category () {
    ...
    const fetchCategory = async () => {
        try {
             ...
             */
            const { data } = await axios.get("https://todos.proen.app.ruk-com.cloud/api/category");
            if (data.status === 200) {
                ...
                <div className="flex flex-col space-y-4">
                    ...
                    <form action="javascript:void()" onSubmit={async (e) => {
                        try {
                             ...
                             * @type {import("axios").AxiosResponse<{ status: number, message: string, data: {} }>}
                             */
                            const { data } = await axios.post("https://todos.proen.app.ruk-com.cloud/api/category", {
...
export function CategoryDetail () {
    ...
    const fetchTodos = async () => {
        try {
             ...
             * @type {import("axios").AxiosResponse<{ status: number, message: string, data: {}[] }>}
             */
            const { data } = await axios.get(`https://todos.proen.app.ruk-com.cloud/api/category/${id}`);
            if (data.status === 200) {
    ...
    const addTodo = async (title, description) => {
        try {
             ...
             * @type {import("axios").AxiosResponse<{ status: number, message: string, data: {} }>}
             */
            const { data } = await axios.post(`https://todos.proen.app.ruk-com.cloud/api/category/${id}`, {
                title,
                description
            });
            if (data.status === 200) {
                        ...
                        <>
                            ...
                            <button className="px-4 py-2 text-white bg-red-500 rounded-md" onClick={async () => {
                                try {
                                     ...
                                     * @type {import("axios").AxiosResponse<{ status: number, message: string, data: {} }>}
                                     */
                                    const { data } = await axios.delete(`https://todos.proen.app.ruk-com.cloud/api/category/${id}`);
                                    if (data.status === 200) {
